...
def plot_data(data_list):
    for data in data_list:
        xmin = 80 * constants.GeV
        xmax = 250 * constants.GeV
        step_size = 5 * constants.GeV
        ...
        signal_color = constants.samples[r'Signal ($m_H$ = 125 GeV)']['color'] # get the colour for the signal bar
        ...
        for s in constants.samples: # loop over samples
            if s not in ['data', r'Signal ($m_H$ = 125 GeV)']: # if not data nor signal
                ...
                mc_colors.append( constants.samples[s]['color'] ) # append to the list of Monte Carlo bar colors
        ...
        lumi_used = str(constants.lumi*constants.fraction) # luminosity to write on the plot
        plt.text(0.05, # x
                0.82, # y
...
# Define the number of process nodes
num_nodes = 12
...
while num_completed_tasks < num_nodes:
    method_frame, header_frame, body = channel.basic_get('done_queue')
    if method_frame:
